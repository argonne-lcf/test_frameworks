...
def main():
    ...
    mesh = DeviceMesh(
        ...
        torch.tensor(
            ...
            .reshape(world_size // args.tp_size, args.tp_size)
            .transpose()
        ),
    )
    if rank == 0:
    ...
    if rank == 0:
        print(
            f"A: {dt_a.shape} created from A_local: {local_tensor_a.shape} with Shard(1)"
        )
        print(
            f"B: {dt_b.shape} created from B_local: {local_tensor_b.shape} with Shard(0)"
        )
    ...
    run(0)
    with profile(
        activities=get_profiler_activities(), record_shapes=True, profile_memory=True
    ) as prof:
