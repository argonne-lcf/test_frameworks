...
BENCH_DIR=/lus/tegu/projects/datasets/software/testing/huihuo_testing_frameworks/test_frameworks/functionality-tests/multi-node/test_incontext_benchmark
...
## RUN ID for PPN=2
RUN_ID=sunspot_tensor_parallel_SEQ${SEQ}_HID${HID}_TP${TP_DEGREE}_NO_SP_NO_USP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")
...
#RUN_ID=sunspot_tensor_parallel_SEQ${SEQ}_HID${HID}_TP${TP_DEGREE}_USP_L${N_LAYERS}_TL${TIMING_LOOPS}_${PRECISION}_${IN_TYPE}_ELEM_${BUCKET}_N${NNODES}_R${NRANKS_PER_NODE}_T${TRIAL}_$(date +"%Y-%m-%d_%H-%M-%S")
...
-bucket ${BUCKET} --logging --log_directory=${BENCH_DIR}/outdir_sunspot/logs/25.190.0-RC3/${RUN_ID} --log_file=${RUN_ID}.log
