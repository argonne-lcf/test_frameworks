...
mpiexec -n ${PALS_WORLD_SIZE} -ppn ${PPN} -l --line-buffer --cpu-bind ${CPU_AFFINITY} \
    -env MASTER_ADDR=$(hostname).hsn.cm.sunspot.alcf.anl.gov \
    -env MASTER_PORT=2345 python test_torch_allgather_no_mpi4py.py
